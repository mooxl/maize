...
const SidebarContent = ({
	...
	return (
		<div
			...
			className={twMerge(
				'flex min-h-0 flex-1 scroll-mb-96 flex-col overflow-auto *:data-sidebar-section:border-l-0 justify-between',
...
const SidebarItem = ({
	...
	const link = (
		<Link
			...
			aria-current={
				routerState.location.pathname === props.href ? 'page' : undefined
			}
			className={composeRenderProps(className, (cls, renderProps) =>
				sidebarItemStyles({
					...renderProps,
					isCurrent: routerState.location.pathname === props.href,
...
const SidebarTrigger = ({
	...
	return (
		...
		>
			{children || (
				<>
					<PanelLeft className="hidden md:inline" data-slot="icon" />
